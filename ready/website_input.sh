...
# website_name= cat reg-test.json | jq  -c -r --arg b "'" '$b + .[] .name + $b'
...
SITE_PATH=$(cat reg-test.json | jq -j --arg p "$SITE_NAME" '.[] | select(.name == "eulachklinik" ) .path')
SITE_URL=$(cat reg-test.json | jq -j --arg p "$SITE_NAME" '.[] | select(.name == "eulachklinik" ) .url')
